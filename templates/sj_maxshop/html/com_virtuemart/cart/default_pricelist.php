...
//     print_r("</pre>");
    ...
    $max= 0;
    foreach($prow->categories as $categorie_id)
    {
        if($max <= $categorie_id) $max=$categorie_id;
    }
    
    $pos = strpos($prow->link,"virtuemart_category_id=");
    $ch = substr($prow->link,0,$pos)."virtuemart_category_id=$max";
    $prow->link = $ch;
	$prow->prices = array_merge($prow->prices,$this->cart->cartPrices[$pkey]);
?>

<tr valign="middle" class="sectiontableentry<?php echo $i ?>">
	<input type="hidden" name="cartpos[]" value="<?php echo $pkey ?>">
	<td class="vm-cart-item-name">
		<?php if ($prow->virtuemart_media_id) { ?>
		<span class="cart-images" style="vertical-align: middle; display: inline-block;">
						 <?php
			if (!empty($prow->images[0])) {
				echo $prow->images[0]->displayMediaThumb ('style="width:80px;height:80px;"', FALSE);
			} 
			?>
		</span>
		<?php } ?>
		...
		echo JHtml::link ($prow->link, $prow->product_name);
