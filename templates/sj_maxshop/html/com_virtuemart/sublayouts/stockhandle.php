...
if (($product->product_in_stock - $product->product_ordered) < 1) {
	...
	<?php
            ...
            echo vmText::_('COM_VIRTUEMART_PRODUCT_AVAILABILITY').': <span class="text-color">'.vmText::_('COM_VIRTUEMART_PRODUCT_OUT_OF_STOCK').'</span>';
...
else if ($product_available_date != '0000-00-00' && $current_date < $product_available_date) {
	?>	<div class="availability">
		<?php echo vmText::_('COM_VIRTUEMART_PRODUCT_AVAILABLE_DATE') .': <span class="text-color">'. JHtml::_('date', $product->product_available_date, vmText::_('DATE_FORMAT_LC4')).'</span>'; ?>
	</div>
<?php
}
    else if (($product->product_in_stock - $product->product_ordered) >= 1){
        echo vmText::_('COM_VIRTUEMART_PRODUCT_AVAILABILITY').': <span class="text-color">'.vmText::_('COM_VIRTUEMART_PRODUCT_IN_STOCK') .'</span>';
    }
?>
