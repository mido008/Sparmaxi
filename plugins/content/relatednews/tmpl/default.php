	...
	foreach ($items as $id => $item) { 
		if($item->id != $article_id){?>
			...
			if ((int)$_params->get('item_date_display', 1) == 1): ?>
				<span class="related-item-date"><?php echo JHtml::date($item->created, 'D, F d, Y'); ?></span>
			...
			if ((int)$_params->get('item_date_display', 1) == 2): ?>
				<div class="related-item-date"><i class="fa fa-clock-o"></i><?php echo JHtml::date($item->created, 'D, F d, Y'); ?></div>
			<?php
			endif;
			?>
			</li>
