...
Class AddElementShortcodes{
	...
	public static function yt_shortcodes_FormElement($element,$name,$desc,$value = ''){
		...
		if(file_exists(dirname(dirname(__FILE__)).DIRECTORY_SEPARATOR.'shortcodes'.DIRECTORY_SEPARATOR.$element.'_item'.DIRECTORY_SEPARATOR.'config.php'))
		{
			...
			$html .= '<input type="button" class="btn btn-info yt_shortcodes_add_element" value="Add '.ucfirst($name).' Item">';
			$html .= '	<div class="yt_shortcodes_son_form_element">';
			
			$html .= '	<div class="yt_shortcodes_son_wrap">';
			$html .= ' 	<input type="button" class="btn yt_shortcodes_son_button" data-active="active" value="'.ucfirst($name).' item 1">';
		...
		jQuery(document).ready(function($){
			...
			$(".yt-generator-insert").click(function(){
				...
				if (window.parent) window.parent.jInsertEditorText(html, $(this).data());
			...
			$(".yt_shortcodes_add_element").click(function(){
				...
				$(".yt_shortcodes_son_form_element .yt_shortcodes_son_wrap").last().append("<input type=\"button\" class=\"btn yt_shortcodes_son_button\" data-active=\"active\" value=\"'.ucfirst($name).' Item "+(dem+1)+"\">");
