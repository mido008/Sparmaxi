...
function testimonial_itemYTShortcode($atts, $content = null){
	...
	if($avatar != '') {
        ...
        $testimonial_avatar .='<img src="' . $avatar . '" alt="'.$atts['author'].'" width="150" height="150" style="border-radius:50%; width:auto; margin:0 auto"/> ';
    };
	...
	$testimonial_item .= '<div class="content_testimon" style="'.($avatar != '' ? "margin-top:15px;" : "").' text-align:center;'.$css.'"><p>'.parse_shortcode(str_replace(array("<br/>", "<br>", "<br />"), " ", $content)).'</p>';
	$testimonial_item .= '<small style="font-weight:bold; ">'.$author.'</small>';
