...
function buttonsYTShortcode($atts, $content = null){
	...
	if($icon != '')
	{
		...
		{
			$btn_icon = "<i class='fa ".$animated_icon." fa-" . trim(str_replace('icon:', '', $icon)) . "'></i> ";
		}else
		{
			$btn_icon = '<img src="'.yt_image_media($icon).'" style="width:18px" alt="" /> ';
		}
	}
	if($icon_right != '')
	{
	...
	return '<a class="yt-clearfix buttons-style-'.$style.' ytbtn'.
			(($type != '') ? ' btn-' . $type : '').
			(($size != '') ? ' btn-' . $size : '') .
			(($full == 'yes') ? ' btn-block' : '') .
			' " style="border-radius:'.$radius.'; '.$background.';'.$dropshadow.'; '.$color.'; '.$center.' ; '.$stylecss.'" href="'.$link.'" target="'.$target.'" data-placement="'.$position.'" title="'.$title.'">'.'<span class="span-buttons"><span>'.$btn_icon. parse_shortcode(str_replace(array("<br/>", "<br>", "<br />", "<p>", "</p>"), " ",$content)) .$icon_right. '</span> '.$subtitle .'</span></a>';
}
