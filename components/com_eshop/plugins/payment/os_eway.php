...
class os_eway extends os_payment
{
	...
	{
        $itemName = JText::sprintf('ESHOP_PAYMENT_FOR_ORDER', $data['order_id']);
		$this->setCustomerFirstname($data['payment_firstname']);
		$this->setCustomerLastname($data['payment_lastname']);
		$this->setCustomerEmail($data['email']);
		$this->setCustomerAddress($data['payment_address_1']);
		$this->setCustomerPostcode($data['payment_postcode']);
		$this->setCustomerInvoiceDescription($itemName);
		$this->setCustomerInvoiceRef($data['order_id']);
		...
		$this->setCardCVN($data['card_cvv']);
		$this->setTrxnNumber($data['order_id']);
		$this->setTotalAmount($data['total'] * 100);
		...
		else
		{
            ...
            $session->set('eshop_payment_error_reason', $this->getErrorMessage());
