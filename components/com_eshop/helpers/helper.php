...
class EshopHelper
{
	...
	public static function sendEmails($row)
	{
		...
		if (self::getConfigValue('invoice_enable') && self::getConfigValue('send_invoice_to_customer'))
		{
			...
			$attachment = JPATH_ROOT . '/media/com_eshop/invoices/' . self::formatInvoiceNumber($row->invoice_number) . '.pdf';
		}
	...
	{
		echo '<div class="copyright" style="text-align:center;margin-top: 5px;"><a href="http://joomdonation.com" target="_blank"><strong>EShop</strong></a> version 1.4.1, Copyright (C) 2012-2013 <a href="http://joomdonation.com" target="_blank"><strong>Ossolution Team</strong></a></div>';
	}
	...
	public static function getProductsNavigation($id)
	{
		...
		{
			$categoryId = self::getProductCategory($id);
			if (!$categoryId)
				$categoryId = 0;
			//Find previous/next products
	...
	{
		$cart = new EshopCart();
		if ($cart->hasDownload() || EshopHelper::getConfigValue('display_price') == 'registered')
		{
			//Only registered
