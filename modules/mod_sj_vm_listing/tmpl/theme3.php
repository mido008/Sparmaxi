...
if(!empty($list)){
	...
	$uniquied='sj_vm_listing'.rand().time();
?>
								...
								<div class="lt-item-inner">
									...
									<div class="lt-summary" >																	  																	  
										...
										<?php  if ($options->item_description_display == 1) {?>									
											<div class="lt-more"  >      
										 		<div class="lt-more-title">											 	
													<a  class=" <?php if($options->link_target=='pop-up'):?>modal<?php endif;?>" href="<?php  echo $item->link; ?>" target="<?php echo $options->link_target;?>" rel="{handler: 'iframe', size: {x: 780, y: 500}, onClose:function(){$('sbox-content').empty();}}"  >
														<?php echo Ytools::truncate($item->product_name,$options->item_title_max_characters) ;?>
													</a>		  													
										 		</div><!-- End sj_item_des_title -->																					
													<?php if( (int)$params->get('price_display', 1) ): ?>
														<div class="lt-more-prices"  >								
															<div class="sale-price" ><?php echo $va= $vm_currency_display->priceDisplay($item->prices['salesPrice']); ?></div>
															<?php if ( $item->prices['discountAmount'] >0 ){
																$price_before_discount = $item->prices['salesPrice'] + $item->prices['discountAmount']; ?>
																<div class="sale-price-before"> <?php echo $vm_currency_display->priceDisplay($price_before_discount); ?></div>
												...
												<?php if ($options->item_readmore_display == 1) {?>   
									  			...
									  			<?php }?>
											</div><!--End sj_item_description-->
										<?php	}?>
									</div> <!--End sj_item_hovercard-->         
								</div><!-- End li -->	
								<?php 							
	...
	<?php ob_start(); ?>
		<?php if($options->item_short_decription_display==0) {?>
			#sj_vm_listing_<?php echo $uniquied ?> .lt-wrap-inner.theme3 .lt-item-inner .lt-icon{
				float:none;
				 padding:0;
			}
			#sj_vm_listing_<?php echo $uniquied ?> .lt-wrap-inner.theme3 .lt-item-inner .lt-icon .lt-icon-inner{
				float:none;
				margin:8px auto;
			}
			
			#sj_vm_listing_<?php echo $uniquied ?> .lt-wrap-inner.theme3 .lt-item-inner .lt-summary{
				text-align:center;
				white-space:nowrap;
				overflow:hidden;
				padding:0;
		...
		<?php }?>
		#sj_vm_listing_<?php echo $uniquied ?> .lt-wrap-inner.theme3 .lt-item .lt-item-inner .lt-more{
			 
			bottom:<?php echo $options->small_image_height + 28?>px;
		}
		<?php if($options->price_display == 0){ ?>	
		#sj_vm_listing_<?php echo $uniquied ?> .lt-wrap-inner.theme3 .lt-more .lt-more-reviews {
			padding:0;
			border:none;
		}
	<?php }?>
