...
if(!class_exists('sj_vm_listing_helper')){
	class sj_vm_listing_helper  {
			...
			if(class_exists('VmModel')){
				...
				if(!empty($category_ids)) {
					foreach ($category_ids as $i => $cat){
						...
						$list[$cat] = $category;
							$category->child = $this->getProductsInCategory($category->virtuemart_category_id, $include_subcategory, $source_group, $source_order, $source_limit, $show_price);
							$products = $category->child;
							if(!empty($products)){
								foreach($products as $j => $product){
									if(array_key_exists($products[$j]->virtuemart_product_id, $custom)){
										$product->link = trim($custom[$product->virtuemart_product_id]->url);
									}
									$product->image = (!empty($product->images[0]->file_url_thumb) && file_exists($product->images[0]->file_url)) ? $product->images[0]->file_url:'modules/'.$module->module.'/assets/images/nophoto.png';
								}
							}
						}
		...
		public function getChildCategories( $cid = 0, $depth = 0 ){
		...
		public function getProductsInCategory( $category_ids = 0, $include_sub = false, $group = null, $order = null, $limit = null, $show_price = true){
			...
			if ( $include_sub === true ){
				count($this->children) or $this->_getCategories();
